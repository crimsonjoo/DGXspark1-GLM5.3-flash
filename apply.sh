...
PORT="${PORT:-18080}"; BACKEND_PORT="${BACKEND_PORT:-18081}"; MODEL_NAME="${MODEL_NAME:-GLM-5.3-Flash-EXL3-2.05}"; MODEL_DIR="${MODEL_DIR:-$HOME/models/GLM-5.3-Flash-exl3-2.05bpw}"; DFLASH_DIR="${DFLASH_DIR:-$HOME/models/GLM-5.3-Flash-DFlash2}"; CACHE_ROOT="${CACHE_ROOT:-$HOME/.cache/glm53-one-spark}"; IMAGE="${IMAGE:-glm53-one-spark:safe-sm121}"
...
SAFE_MHC="${SAFE_MHC:-1}"; PREFIX_CACHE="${PREFIX_CACHE:-0}"; SAFE_MAX_OUTPUT_TOKENS="${SAFE_MAX_OUTPUT_TOKENS:-8192}"; GATEWAY_MAX_BODY_BYTES="${GATEWAY_MAX_BODY_BYTES:-16777216}"; GATEWAY_BACKEND_TIMEOUT="${GATEWAY_BACKEND_TIMEOUT:-1800}"
for x in PORT BACKEND_PORT CTX SEQS MAX_BATCHED_TOKENS K READY_TIMEOUT SAFE_MAX_OUTPUT_TOKENS GATEWAY_MAX_BODY_BYTES GATEWAY_BACKEND_TIMEOUT; do [[ "${!x}" =~ ^[0-9]+$ ]] || die "Invalid $x=${!x}"; done
[ "$PORT" != "$BACKEND_PORT" ] || die 'PORT and BACKEND_PORT must differ'
...
if ss -lnt 2>/dev/null | awk 'NR>1{print $4}' | grep -Eq "(^|:)$PORT$" && ! docker ps -a --format '{{.Names}}' | grep -Eqx 'glm53-api-gateway|glm53-one-spark'; then die "Port $PORT is used by another process"; fi
export PORT BACKEND_PORT MODEL_NAME MODEL_DIR DFLASH_DIR CACHE_ROOT IMAGE CTX SEQS GPU_MEM MAX_BATCHED_TOKENS K SAFE_MHC PREFIX_CACHE SAFE_MAX_OUTPUT_TOKENS GATEWAY_MAX_BODY_BYTES GATEWAY_BACKEND_TIMEOUT WATCHDOG WATCHDOG_INTERVAL WATCHDOG_FAILURES WATCHDOG_STARTUP_GRACE WATCHDOG_DEEP_CHECK WATCHDOG_DEEP_INTERVAL WATCHDOG_DEEP_TIMEOUT WATCHDOG_DEEP_FAILURES WATCHDOG_MAX_RESTARTS WATCHDOG_RESTART_WINDOW
...
if [ "$WATCHDOG" = 1 ]; then "${compose[@]}" --profile watchdog up -d --build "${args[@]}"; else "${compose[@]}" --profile watchdog rm -sf watchdog >/dev/null 2>&1 || true; "${compose[@]}" up -d --build "${args[@]}" glm gateway; fi
...
printf 'Gateway logs   : docker logs -f glm53-api-gateway\nEngine logs    : docker logs -f glm53-one-spark\nStatus         : ./stop.sh --status\nApply settings : ./apply.sh\nStop           : ./stop.sh\nConfig         : %s\n\n' "$CONFIG_FILE"
